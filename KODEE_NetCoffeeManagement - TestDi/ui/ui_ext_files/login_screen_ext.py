...
class login_screen_ext(Ui_MainWindow):
    def setupUi(self, MainWindow):
        ...
        # Set password fields to display dots instead of actual characters
        self.lineEdit_EMPassword.setEchoMode(QLineEdit.EchoMode.Password)
        self.lineEdit_MNPassword.setEchoMode(QLineEdit.EchoMode.Password)
    ...
    def login(self):
        ...
        if not employee_id or not password:
            self.show_error("Please enter all the required information.")
            return
        
        # Kiểm tra cú pháp ID
        if not self.validate_id_format(employee_id):
            self.show_error("Invalid Employee ID\n Employee ID: EM01, EM02,... \nManager ID: MN01, MN02,...")
            return
    ...
    def show_error(self, message):
        ...
        msg_box = QMessageBox.critical(self.MainWindow, "Login error", message)
